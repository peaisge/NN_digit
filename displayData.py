...
def displayData(X, example_width = 1):
    ...
    #Setup black display
    display_array = -np.ones((pad + display_rows * (example_height+pad), pad + display_cols * (example_width + pad)), float);
    curr_ex = 0
    for j in range(display_rows):
        for i in range(display_cols):
            if curr_ex > (m-1):
                break
            x = pad + j * (example_height + pad) + np.arange(example_height)
            y = pad + i * (example_width + pad) + np.arange(example_width)
